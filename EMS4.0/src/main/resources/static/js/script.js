...
const toggleSidebar = () => {
    ...
    if (sidebar.is(":visible")) {
        ...
        sidebar.animate({ width: "0px", opacity: 0 }, 300, function () {
            $(this).hide(); // Completely hide after animation
        });
        content.animate({ marginLeft: "0%" }, 300);
    } else {
        // Show sidebar smoothly
        sidebar.css({ display: "block", opacity: 0, width: "0px" })
               .animate({ width: "20%", opacity: 1 }, 300);
        content.animate({ marginLeft: "21%" }, 300);
    }
};
...
rzp.on('payment.failed', function (response){
 ...
 Swal.fire("Payment Pending", "Your payment is being processed. Please wait a moment.", "info");
});
rzp.open();
			}
		},
		...
		{
			console.log(error)
			Swal.fire("Transaction Failed!", "Payment could not be processed. Please try again later.", "error");
		},
...
function updatePaymentOnServer(payment_id, order_id, status) {
	// Show spinner before making AJAX call
	...
	$.ajax({
		...
		dataType: "json",
		success: function(response) {
			$('#spinner-overlay').hide(); // Hide spinner after success
			Swal.fire({
				icon: 'success',
