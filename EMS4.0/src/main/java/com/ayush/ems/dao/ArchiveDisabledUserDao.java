...
public interface ArchiveDisabledUserDao extends JpaRepository<ArchiveDisabledUser, Integer> {
	@Query(value = "select count(1) from archive_disabled_user", nativeQuery = true)
	public int getArchiveUserCount();

	@Query(value = "select u.sno from archive_disabled_user u order by u.sno desc limit 1", nativeQuery = true)
