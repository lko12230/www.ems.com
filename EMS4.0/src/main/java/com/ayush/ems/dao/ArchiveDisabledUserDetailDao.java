...
public interface ArchiveDisabledUserDetailDao extends JpaRepository<ArchiveDisabledUserDetail, Integer> {
	@Query(value = "select count(1) from archive_disabled_user_detail", nativeQuery = true)
	public int getArchiveUserCount();

	@Query(value = "select u.sno from archive_disabled_user_detail u order by u.sno desc limit 1", nativeQuery = true)
