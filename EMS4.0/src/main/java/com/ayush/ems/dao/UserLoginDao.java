...
public interface UserLoginDao extends JpaRepository<UserLoginDateTime, Integer> {
	...
	@Query("SELECT u FROM UserLoginDateTime u WHERE (u.company_id = :company_id OR u.team_id = :team_id) AND (u.team_id not in ('No Record Found',' ') OR u.company_id not in ('No Record Found',' '))")
	List<UserLoginDateTime> findAllByCompanyIdOrTeamId(@Param("company_id") String companyId,
			@Param("team_id") String teamId);

	// Update session interruption status for a specific session ID
	@Modifying
	@Transactional
	@Query(value = "UPDATE employee_login_record u SET u.is_session_interrupted = '1', u.user_status='0', u.logout_date_and_time = CURRENT_TIMESTAMP WHERE u.id = ?1 ORDER BY u.login_date_and_time DESC LIMIT 1", nativeQuery = true)
	...
	// Check if a specific session has been manually logged out by getting the
	// latest matching record for a session ID
	...
	// Update session interruption status for a specific session ID
	@Modifying
	@Query(value = "UPDATE employee_login_record u SET u.is_session_interrupted = '1', u.user_status='0',u.logout_date_and_time = CURRENT_TIMESTAMP WHERE u.email = ?1 and  u.logout_date_and_time IS NULL ORDER BY u.login_date_and_time DESC LIMIT 1", nativeQuery = true)
	void updateSessionInterruptedStatusBrowserClosed(String email);
