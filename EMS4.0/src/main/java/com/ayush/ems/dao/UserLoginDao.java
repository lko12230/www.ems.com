...
public interface UserLoginDao extends JpaRepository<UserLoginDateTime, Integer> {
 ...
 // Check if a specific session has been manually logged out by getting the latest matching record for a session ID
	...
	@Query(value="select count(1) from railway.employee_login_record",nativeQuery = true)
	public int getLoginCount();
	
	 // Update session interruption status for a specific session ID
    @Modifying
    @Query(value = "UPDATE employee_login_record u SET u.is_session_interrupted = '1', u.user_status='0',u.logout_date_and_time = CURRENT_TIMESTAMP WHERE u.email = ?1 ORDER BY u.login_date_and_time DESC LIMIT 1", nativeQuery = true)
