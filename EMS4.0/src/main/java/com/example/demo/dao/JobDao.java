...
public interface JobDao extends JpaRepository<Job, Integer> {
	...
	@Query("select u.id from database_ems.Job u order by id desc limit 1")
	public int getJobLastId();
	
	@Query(value="select count(1) from database_ems.Job u order by id desc limit 1",nativeQuery = true)
